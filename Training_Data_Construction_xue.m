...
function train_set = Training_Data_Construction_xue(knowledge, sources, K_value, popsize, FEsMax, alpha, para_train_set_fix, statistics)
...
% label_fuzzy=zeros(num_sample, 1);  % the quantified degree of performance improvement
label=zeros(num_sample, 2);  % the quantified degree of performance improvement
label_fuzzy=zeros(num_sample, 2); % the label set of binary labels
...
% if fitnuss_min <= min(knowledge(target_no).fitnesses{Current_g, 1})
%     label_fuzzy(i)=1;  % positive class
% else
%     label_fuzzy(i)=2;  % negative class
% end
% source task to target task
if fitnuss_min > min(knowledge(target_no).fitnesses{1, 1})
    label(i, 1)=0;
    label_fuzzy(i, 1)=2;  % negative class
elseif fitnuss_min <= min(knowledge(target_no).fitnesses{Current_g, 1})
    label(i, 1)=Current_g;
    label_fuzzy(i, 1)=1;  % positive class
else
    count = Current_g-1;
    while count
...
label_fuzzy(i, 2)=Current_g;  
if length(find(label_fuzzy(:, 1)==1))>=num_sample/2 && length(find(label_fuzzy(:, 1)==2))>=num_sample/2
    break;
end
...
% value=median(label(:, 1));  % set the median of labels as one threshold value
% label_fuzzy=zeros(num_sample, 1); % the label set of binary labels
% for nn=1:num_sample
%    if label(nn, 1)>value || label(nn, 1)==Gen
...
samples_p = find(label_fuzzy(:, 1)==1);
order1 = randperm(length(samples_p));
samples_n = find(label_fuzzy(:, 1)==2);
order2 = randperm(length(samples_n));
train_set = [train_set(samples_p(order1(1:num_sample/2)), :); train_set(samples_n(order2(1:num_sample/2)), :)];
% ss=randperm(num_sample);
% train_set= train_set(ss, :);
temp = [label_fuzzy(samples_p(order1(1:num_sample/2)), 2); label_fuzzy(samples_n(order2(1:num_sample/2)), 2)];
% [~, index_s] = sort(temp, 'ascend');
index_s = randperm(num_sample);
train_set= train_set(index_s, :);
% train_set = [train_set set_gen];
end
